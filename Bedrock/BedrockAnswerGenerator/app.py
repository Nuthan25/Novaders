...
BUCKET = os.getenv('BUCKET_NAME_IN')
...
def get_prompt_qry_resp(result, question):
    result = result if result is not None else ""
    question = question if question is not None else ""

    with open('query_response_prompt.txt', 'r') as file:
        prompt = file.read().replace("$$RESULT$$", result).replace("$$QUESTION$$", question)
    return prompt
...
def generate_html_response(event_params):
    ...
    doc_type = "note"
    question_embedding = embedd_question(question, model_name, doc_type)
    ...
    info_note = get_addition_info_text(job_id, org_id)
    # if check_query_index(index, model_name, doc_type, client):
    #     note = query_shared_index(index,model_name, doc_type, question_embedding, client, size)
    # Format the response
    result_str = str(result)
    html_prompt = html_prompt.replace('$$RESULT$$', str(result_str)).replace('$$QUESTION$$', str(question)).replace('$$NOTE$$', str(info_note))
    ...
    if is_table_possible:
        try:
            ...
            logger.error(f"Problematic JSON: {ans}")
...
def handle_neptune_errors(query_result, bucket, org_id, job_id, question):
    ...
    prompt = get_prompt_qry_resp(str(query_result), question)
...
def send_message_to_queue(connection_id, message_to_queue, org_id):  # Send message to dispatcher queue
    ...
    # Store original answer for later use
    original_answer = message_to_queue.get('answer')

    # Compress the original answer for type 4 message
    compressed_data = gzip.compress(json.dumps(original_answer).encode('utf-8'))
    compressed_answer = base64.b64encode(compressed_data).decode('utf-8')

    try:
        # First send message with type 7 and empty answer
        message_to_queue_type7 = message_to_queue.copy()
        message_to_queue_type7['answer'] = ""  # Empty answer for type 7

        message = {
            "target": [connection_id],
            "data": {
                "message": message_to_queue_type7,
                'type': 7
            }
        }
        ...
        logger.debug("Type 7 message pushed")
        logger.info(json.dumps({"log_type": "sqs", "value": org_id}))

        # Then send message with type 4 with compressed answer
        message_to_queue_type4 = message_to_queue.copy()
        message_to_queue_type4['answer'] = compressed_answer

        message = {
            "target": [connection_id],
            "data": {
                "message": message_to_queue_type4,
                'type': 4
            }
        }
        sqs_client.send_message(QueueUrl=queue_url,
        ...
        logger.debug("Type 4 message pushed")
        logger.info(json.dumps({"log_type": "sqs", "value": org_id}))
