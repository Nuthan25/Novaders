...
from query_retrieve import query_shared_node_index, search_similar_edges, query_shared_index, query_shared_index_like, check_query_index, search_relevant_node
...
def get_text_embedding(text: str, dimensions, region: str = REGION):
    """
...
def embedd_question(question, model_name, type):
    try:
        ...
        logger.info(f"Error creating embedd question: {e}")
        return "error"
...
def retrieve_similar_nodes_from_bedrock(question, json_nodes):
    try:
        with open("node_retrieve_prompt.txt", "r") as file:
                prompt = file.read().replace("$$question$$", question).replace("$$nodes$$",str(json_nodes))
        similar_nodes = generate_response(prompt)
        return similar_nodes
    except Exception as e:
        print(f"Error getting similar nodes: {e}")
        return e

def check_null(similar_nodes):
    labels = []
    try:
        if "None" in similar_nodes:
                similar_nodes = similar_nodes.replace("None", "null")
        data = json.loads(similar_nodes)
        for label in data:
    ...
    if isinstance(result, str):
        try:
            ...
            if isinstance(parsed, list) and all(isinstance(item, dict) for item in parsed):
                print("list of JSON (from string)")
                return parsed
            elif isinstance(parsed, dict):
                print("single JSON object (from string)")
                return parsed
            else:
                print("plain string or other JSON type")
                return "plain string"
...
# Main parsing logic for similar_nodes
def parse_similar_nodes(similar_nodes):
    labels = []
    if similar_nodes and similar_nodes.strip():
        data = check_result_type(similar_nodes)
        if data != "plain string":
            for label in data:
                labels.append(label.get("label"))
            return labels
        else:
        ...
        print("similar_nodes is empty or only whitespace.")
        return "No similar data found"


def retrieve_similar_nodes(question, client, model_id, index):
    """
    Retrieve similar nodes based on question
    ...
    try:
        ...
        question_embedding = get_text_embedding(question, 512)
        embed_end = time.time()
        logger.info(f"Time taken to generate embeddings: {embed_end - embed_start} seconds")

        # Search for relevant nodes using hybrid search
        search_start = time.time()
        similar_nodes = search_relevant_node(index, model_id, question_embedding, client,
                                                            question_text=question)
        search_end = time.time()
        logger.info(f"Time taken to search nodes: {search_end - search_start} seconds")

        logger.info(f"Similar nodes:-- {similar_nodes}")
        return similar_nodes
    except Exception as e:
...
def lambda_handler(event, context):
    ...
    try:
        ...
        # json_nodes = retrieve_similar_nodes(question, client, model_name, index)
        similar_nodes = retrieve_similar_nodes_from_bedrock(question, json_nodes)
        print("similar_nodes_frm_claude:-", similar_nodes)
        labels = parse_similar_nodes(similar_nodes)
        if labels != "No similar data found":
            ...
            json_node_data = check_result_type(similar_nodes)
            schema = parse_json_nodes(json_node_data)
            ...
            if not check_query_index(index, model_name, 'like', client):
                ...
                if check_query_index(index, model_name, 'file', client):
                    ...
                    query = query_response(job_id, file_response, None, lmtn_prompt, question, schema, relation,
                                           note_response)
                    logger.info("Got DATA from Files")
                else:
                    logger.info("No Like Query or files data")
                    query = query_response(job_id, None, None, lmtn_prompt, question, schema, relation,
                                           note_response)
                ...
                if check_question_aoss(question, like_response):
                    ...
                    logger.info(f"got Liked query")

                elif check_query_index(index, model_name, 'file', client):
                    logger.info("Using File data")
                    question_embed_file = embedd_question(question, model_name, "file")
                    file_response = query_shared_index(index, model_name, "file", question_embed_file, client, 10)
                    print(json.dumps(file_response, indent=2))
                    query = query_response(job_id, file_response, None, lmtn_prompt, question, schema, relation,
                                           note_response)
                    logger.info("**Got DATA from Files**")
            ...
            if ((query and ("MATCH" in query or "RETURN" in query)) or query_rag):
                logger.info("got query")
                if query:
                    print("query", query)
                if query_rag:
                    print("query_rag", query_rag)
                match_queries = None
